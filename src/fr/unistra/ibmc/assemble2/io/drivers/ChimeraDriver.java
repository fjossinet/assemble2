...
public class ChimeraDriver extends AbstractTertiaryViewerDriver {
    ...
    public ChimeraDriver(final Mediator mediator) {
        super(mediator, AssembleConfig.getChimeraPath());
        ...
        try {
            ...
            if (e.getMessage().startsWith("Cannot run program")) {
                ...
                texts.add("I'm not able to launch Chimera.");
                nextActions.add(new MessagingSystemAction() {
                    @Override
                    public void run() {
                        try {
                            mediator.getAssemble().getConfigureAssemble().doClick();
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
                ...
                texts.add("Please check the location of your Chimera executable.");
    ...
    public void importStructure(int modelID, int relativeID) throws Exception {
        File f = IoUtils.createTemporaryFile("export.pdb");
    ...
    public void synchronizeFrom() throws Exception {
        mediator.getSecondaryCanvas().getMessagingSystem().addSingleStep("Please wait, i'm synchronizing with Chimera...", null, null);
        mediator.getSecondaryCanvas().getActivityToolBar().startActivity(null);
        File f = IoUtils.createTemporaryFile("export.pdb");
        PrintWriter commandInput = new PrintWriter((new OutputStreamWriter(new BufferedOutputStream(process.getOutputStream()))), true);
        ...
        TertiaryStructure newTS = new TertiaryStructure(mediator.getSecondaryStructure().getMolecule());
        if (mediator.getTertiaryStructure() != null)
            newTS.setId(mediator.getTertiaryStructure().getId());
        mediator.setTertiaryStructure(newTS);
        mediator.getSecondaryStructure().setLinkedTs(newTS);
        for (TertiaryStructure ts:structures)
            for (Residue3D residue3D:ts.getResidues3D()) {
                ...
                for (Residue3D.Atom a: residue3D.getAtoms())
    ...
    public void synchronizeTo() throws Exception {
        File f = IoUtils.createTemporaryFile("model.pdb");
        FileParser.writePDBFile(mediator.getTertiaryStructure().getResidues3D(), true, new PrintWriter(f));
    ...
    public void addFragment(File f, List<Residue3D> computedResidues, List<Residue3D> previousResidues, boolean firstFragment) {
        ...
        if (toBedeleted.size() == 2 && mediator.getSecondaryStructure().getPairedResidueInSecondaryInteraction(mediator.getSecondaryStructure().getResidue(toBedeleted.get(0).getAbsolutePosition())).getAbsolutePosition() == toBedeleted.get(1).getAbsolutePosition()) {
            anchorResidue1 = toBedeleted.get(0).getAbsolutePosition();
    ...
    public void color3D(List<Residue> residues) {
        final StringBuffer command = new StringBuffer();
        for (Residue r: residues)
            command.append("color "+((float)r.getFinalColor(mediator.getSecondaryCanvas().getGraphicContext()).getRed()/(float)255)+","+(float)r.getFinalColor(mediator.getSecondaryCanvas().getGraphicContext()).getGreen()/(float)255+","+(float)r.getFinalColor(mediator.getSecondaryCanvas().getGraphicContext()).getBlue()/(float)255+","+(float)r.getFinalColor(mediator.getSecondaryCanvas().getGraphicContext()).getAlpha()/(float)255+" #0,2:"+r.getAbsolutePosition()+"; ");
